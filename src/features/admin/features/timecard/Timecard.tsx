  ...
  const handleClockIn = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        insertDataIntoCollection('Timecards', newTimecard);
      },
      (error) => {
        ...
        dispatch(
          setEntireNotification({
            notificationOpen: true,
            notificationSeverity: 'error',
            notificationMessage: 'There was an error clocking you in. Be sure you have location services enabled.',
          })
        );
      }
    );
...
const handleClockOut = useCallback(() => {
...
}, [dispatch, timecard.TimecardInfo.ClockIn]);
  ...
  if (timecard.Active) {
    return (
      <Block tailwindClasses="flex flex-col items-center justify-center p-6 bg-gray-100 rounded-2xl shadow-xl w-full max-w-md mx-auto mt-10">
        ...
        <Block tailwindClasses="text-4xl font-mono text-primary mb-2">{localTime}</Block>
        <Block tailwindClasses="text-lg text-gray-700 mb-6">Elapsed Time: {elapsedTime}</Block>
        <Block
          as="button"
          children="Clock Out"
  ...
  return (
    <Block tailwindClasses="flex flex-col items-center justify-center p-6 bg-gray-100 rounded-2xl shadow-xl w-full max-w-md mx-auto mt-10">
      ...
      <Block tailwindClasses="text-4xl font-mono text-primary mb-6">{localTime}</Block>
      <Block
        as="button"
        children="Clock In"
