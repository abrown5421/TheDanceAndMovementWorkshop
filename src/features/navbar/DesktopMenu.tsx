...
const DesktopMenu: React.FC = () => {
  ...
  return (
    <nav ref={containerRef} className="relative flex items-center gap-6 font-sans text-base text-gray-800">
      {viewport.type === 'desktop' && !dashboard.dashboardMode ? (
        <>
        ...
        <IconButton color="text-primary" ariaLabel="Favorite" onClick={() => {
          ...
          dispatch(setDrawerState({ key: 'drawerTitle', value: `Good ${getTimeOfDay()}` + (dashboard.dashboardMode ? `, ${auth.user?.UserFName}` : '') }));
        }}>
          <Hamburger />
        </IconButton>
      )}
